...
dataframe = pandas.read_csv('../Data/international-airline-passengers.csv', usecols=[1], engine='python',
                            skipfooter=3)  # 原有两列，一列是时间，一列是乘客数量，这里利用usecols=[1],只取了乘客数量一列
...
def create_dataset(dataset, look_back=1):
    ...
    for i in range(len(dataset) - look_back - 1):  # 此处是否要去掉-1呢？待定中
        a = dataset[i:(i + look_back), 0]  # i:i+look_back代表用预测的时间点前的look_back点来作为输入，此处用前1个点作为输入
        dataX.append(a)
